...
document.getElementById('ownerFormm').addEventListener('submit', function(event) {
    ...
    window.location.href = 'commerical3.html';

});



document.getElementById('clientAgencyPhone').addEventListener('input', function(event) {
...
document.getElementById('commercialClientPhone').addEventListener('input', function(event) {
    const value = event.target.value;
    const idRegex = /^05\d*$/;
    if (value.length > 1 && !idRegex.test(value)) {
        alert('يجب أن يبدأ رقم الجوال بـ 05 ويحتوي على أرقام فقط.');
...
document.getElementById('commercialName').addEventListener('input', function(event) {
...
document.getElementById('commercialClientID').addEventListener('input', function(event) {
    const idRegex = /^1\d*$/;
    const value = event.target.value;
    const lastChar = value.slice(-1);
